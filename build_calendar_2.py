...
Please note the following didactics calendar is intended as a preview of the scheduled didactics for UCI Dermatology and is issued here up to two months in advance.
...
def main() -> None:
    ...
    # show two full months ahead
    one_month_ahead = next_month_same_day(today)
    window_end   = next_month_same_day(one_month_ahead)     # inclusive same-day 2 months out

    events = [e for e in events if window_start <= e["date"] <= window_end]
    if not events:
        sys.exit("No events found in this 2-month window.")
